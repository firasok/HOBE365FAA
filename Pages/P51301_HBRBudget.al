page 51301 HBRBudget
//<NCO>
// Copy of page 113 Budget, changed for HBR purposes
//</NCO>
{
    Caption = 'HBR Budget';
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                //<NCO>
                field(BudgetCompareBaseYear; BudgetCompareBaseYear)
                {
                    ApplicationArea = all;
                    Caption = 'Base Year for Budget Compare';
                    ToolTip = 'HBR: Specifies wich year the budget regulation should be based on.';
                    BlankZero = true;
                    trigger Onvalidate()
                    begin
                        if BudgetCompareBaseYear <> 0 then begin
                            if (BudgetCompareBaseYear > 2100) or (BudgetCompareBaseYear < 2019) then
                                Error(BaseYearRangeErrorTxt);
                        END;
                        FindAmountAdjFactors();
                        UpdateMatrixSubform();
                    end;
            ...
            part(MatrixForm; "HBRBudget Matrix")
            {
                ApplicationArea = Suite;
            }
            group(Filters)
    ...
    actions
    {
        area(navigation)
        {
...

            {
                Caption = 'Rates';
                //Image = price;
                action(HBRBudgetRegulationRates)
                {
                    ApplicationArea = Suite;
                    Caption = 'Budget Regulation Rates';
                    Image = Price;
                    Promoted = true;
                    PromotedCategory = "Report";
                    ToolTip = 'Open a list of the budget regulation rates.';

                    trigger OnAction()

                    begin
                        PAGE.Run(PAGE::"HBR Budget Regulation Rates");
                    end;
                }
    ...
    var
        ...
        //<NCO>
        RegulateAmounts: Boolean;
        BudgetCompareBaseYear: Integer;
        AmountAdjustFactorP: Decimal;
        AmountAdjustFactorL: Decimal;
        AmountAdjustFactorPL: Decimal;
        BaseYearRangeErrorTxt: Label 'Budget base Year should be betweeen 2019 and 2100';
    ...
    begin
        ...
        BudgetCompareBaseYear := GLBudgetName."Budget Regulation Base Year";
        FindAmountAdjFactors();
        BudgetLocked := (GLBudgetName.Locked and not hbrsetup.IsBudgetResponsible(userid));
        //</NCO>
    ...
    begin
        CurrPage.MatrixForm.PAGE.Load(
          ...
          RegulateAmounts, AmountAdjustFactorP, AmountAdjustFactorL, AmountAdjustFactorPL, BudgetLocked);
    ...
    local procedure FindAmountAdjFactors()
    var
        BudgetNameRec: Record "G/L Budget Name";
        BudgetRegulationRec: Record "HBR Budget Regulation Rate";
    Begin
        BudgetNameRec.get(BudgetName);
        if BudgetCompareBaseYear = 0 then
            RegulateAmounts := false
        ELSE
            BudgetNameRec.TestField("Budget Regulation Base Year");
        RegulateAmounts := true;

        if RegulateAmounts then
            BudgetRegulationRec.GetRegulationRates(BudgetNameRec."Budget Regulation Base Year", BudgetCompareBaseYear, AmountAdjustFactorP, AmountAdjustFactorL, AmountAdjustFactorPL);
        if RegulateAmounts then
            RegulateAmounts := (AmountAdjustFactorL <> 1) AND (AmountAdjustFactorP <> 1) and (AmountAdjustFactorPL <> 1);
    End;
}
