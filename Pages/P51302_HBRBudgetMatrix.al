page 51302 "HBRBudget Matrix"
//<NCO>
// Copy of page 9203 Budget Matrix, changed for HBR purposes
//</NCO>
{
            ...
            repeater(Control1)
            {
                ...
                field(TotalBudgetedAmount; Amount)
                {
                    ...
                    begin
                        ...
                        BudgetDrillDown(false);
                    end;
                }
                //<NCO>
                field(TotalDeviationAmount; "Deviation Amount")
                {
                    ApplicationArea = Suite;
                    AutoFormatExpression = FormatStr;
                    AutoFormatType = 10;
                    BlankZero = true;
                    Caption = 'Deviation Amount';
                    Editable = false;
                    Style = Strong;
                    StyleExpr = Emphasize;
                    ToolTip = 'Specifies the G/L account''s total Deviation.';

                    trigger OnDrillDown()
                    begin
                        SetCommonFilters(GLAccBudgetBuf);
                        SetDimFilters(GLAccBudgetBuf, 0);
                        BudgetDrillDown(true);
                    end;
                }
                //</NCO>
    ...
    begin
        ...
        "Deviation Amount" := MatrixMgt.RoundValue(CalcDeviationAmount(False), RoundingFactor);
        FormatLine;
    end;
    ...
    local procedure BudgetDrillDown(DeviationOnly: Boolean)
    var
    ...
    begin
        ...
        if DeviationOnly then
            GLBudgetEntry.SetRange("Budget Deviation Entry", true);
        GLBudgetEntries.Editable(true);
    ...
    begin
        ...
        //<NCO>
        if not AdjustAmounts then begin
            GLAccBudgetBuf.CalcFields("Budgeted Amount");
            RegulatedAmount := GLAccBudgetBuf."Budgeted Amount";
        end else begin
            GLAccountRec.SetFilter("No.", GLAccBudgetBuf.getfilter("G/L Account Filter"));
            if not GLAccountRec.FindSet() then begin
                GLAccBudgetBuf.CalcFields("Budgeted Amount");
                RegulatedAmount := GLAccBudgetBuf."Budgeted Amount";
            end ELSE BEGIN
                RegulatedAmount := 0;
                repeat
                    GLAccBudgetBuf.SetFilter("G/L Account Filter", GLAccountRec."No.");
                    case GLAccountRec."Budget Regulation Type" of
                        GLAccountRec."Budget Regulation Type"::L:
                            AdjFactor := lRate;
                        GLAccountRec."Budget Regulation Type"::P:
                            AdjFactor := pRate;
                        GLAccountRec."Budget Regulation Type"::PL:
                            AdjFactor := plRate;
                        else
                            AdjFactor := 1
                    end; //CASE
                    GLAccBudgetBuf.CalcFields("Budgeted Amount");
                    RegulatedAmount += AdjFactor * GLAccBudgetBuf."Budgeted Amount";
                until GLAccountRec.next = 0;
            end;

            exit(RegulatedAmount);
        end;
        exit(RegulatedAmount);
        //</NCO>
    end;
    ...
    local procedure CalcDeviationAmount(SetColumnFilter: Boolean): Decimal
    //<NCO>
    VAR
        GLAccountRec: record "G/L Account";
        AdjFactor: Decimal;
        RegulatedDevAmount: Decimal;
    begin
        SetCommonFilters(GLAccBudgetBuf);
        SetDimFilters(GLAccBudgetBuf, 0);
        if SetColumnFilter then
            SetDimFilters(GLAccBudgetBuf, 1);
        if not AdjustAmounts then begin
            GLAccBudgetBuf.CalcDevAmount();
            RegulatedDevAmount := GLAccBudgetBuf."Deviation Amount";
        end else begin
            GLAccountRec.SetFilter("No.", GLAccBudgetBuf.getfilter("G/L Account Filter"));
            if not GLAccountRec.FindSet() then begin
                GLAccBudgetBuf.CalcDevAmount();
