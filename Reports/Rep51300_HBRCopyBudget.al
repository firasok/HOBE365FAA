report 51300 "HBR Copy G/L Budget"
{
    ...
    begin
        ...
        if not GLBudgetName.FindFirst then begin
            if not NoMessage then
                if not ConfirmManagement.GetResponse(StrSubstNo(Text005, ToGLBudgetName), true) then
                    Continue := false;
        ...
        if (not NoMessage) and Continue then
            if not ConfirmManagement.GetResponse(Text006, true) then
                Continue := false;
    ...
    var
        Sign: Decimal;
        AmountAdjustFactorP: Decimal;
        AmountAdjustFactorL: Decimal;
        AmountAdjustFactorPL: Decimal;
        GLAccountRegulation: Record "G/L Account";
        RegulateToYear: Integer;
        RegulateFromYear: Integer;
        GLbudgetNameRegulation: Record "G/L Budget Name";
        BudgetRegulationRate: record "HBR Budget Regulation Rate";
    begin
        if TempGLBudgetEntry.Find('-') then begin
            GLbudgetNameRegulation.get(FromGLBudgetName);
            GLbudgetNameRegulation.TestField("Budget Regulation Base Year");
            RegulatefromYear := GLbudgetNameRegulation."Budget Regulation Base Year";

            GLbudgetNameRegulation.get(ToGLBudgetName);
            GLbudgetNameRegulation.TestField("Budget Regulation Base Year");
            RegulateToYear := GLbudgetNameRegulation."Budget Regulation Base Year";
            AmountAdjustFactorL := BudgetRegulationRate.GetRegulationRate(BudgetRegulationRate."Budget Regulation Type"::L, RegulateFromYear, RegulateToYear);
            AmountAdjustFactorP := BudgetRegulationRate.GetRegulationRate(BudgetRegulationRate."Budget Regulation Type"::P, RegulateFromYear, RegulateToYear);
            AmountAdjustFactorPL := BudgetRegulationRate.GetRegulationRate(BudgetRegulationRate."Budget Regulation Type"::PL, RegulateFromYear, RegulateToYear);
            ...
            repeat
                if TempGLBudgetEntry.Amount <> 0 then begin
                    ...
                    if GLAccountRegulation."No." <> ToGLBudgetEntry."G/L Account No." then begin
                        GLAccountRegulation.get(ToGLBudgetEntry."G/L Account No.");
                    END;

                    CASE GLAccountRegulation."Budget Regulation Type" OF
                        GLAccountRegulation."Budget Regulation Type"::"P":
                            BEGIN
                                ToGLBudgetEntry.Amount := Round(TempGLBudgetEntry.Amount * AmountAdjustFactorP);
                            END;
                        GLAccountRegulation."Budget Regulation Type"::"L":
                            BEGIN
                                ToGLBudgetEntry.Amount := Round(TempGLBudgetEntry.Amount * AmountAdjustFactorL);
                            END;
                        GLAccountRegulation."Budget Regulation Type"::"PL":
                            BEGIN
                                ToGLBudgetEntry.Amount := Round(TempGLBudgetEntry.Amount * AmountAdjustFactorPL);
                            END;
                    END; //CASE
    ...
    procedure Initialize(FromSource2: Option; FromGLBudgetName2: Code[10]; FromGLAccountNo2: Code[250]; FromDate2: Text[30]; ToGlBudgetName2: Code[10]; ToGLAccountNo2: Code[20]; ToBUCode2: Code[20]; RoundingMethod2: Code[10]; DateAdjustExpression2: DateFormula; NoMessage2: Boolean)
    begin
    ...
    procedure InitializeRequest(FromSource2: Option; FromGLBudgetName2: Code[10]; FromGLAccountNo2: Code[250]; FromDate2: Text[30]; FromClosingEntryFilter2: Option; DimensionText: Text[250]; ToGlBudgetName2: Code[10]; ToGLAccountNo2: Code[20]; RoundingMethod2: Code[10]; DateAdjustExpression2: DateFormula; ToDateCompression2: Option)
    begin
        FromSource := FromSource2;
        FromGLBudgetName := FromGLBudgetName2;
        FromGLAccountNo := FromGLAccountNo2;
